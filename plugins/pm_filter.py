     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/AstroBotSupport")
        ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ᴏᴡɴᴇʀ ɢɪᴛʜᴜʙ', url="https://github.com/DevAstrro")
        ],[
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/DevAstrro")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        await query.message.edit_text(
            text="🔊 <b>ᴛᴇxᴛ ᴛᴏ sᴘᴇᴇᴄʜ</b>\n\nSend /tts followed by your text to convert it to audio.\n\n<b>Example:</b> <code>/tts Hello, how are you?</code>",
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )

    elif query.data == "carbon_help":
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help")
        ]]
        reply_markup = InlineKeyboardMarkup(btn)
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/DevAstrro1")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
@Client.on_message(filters.command("tts") & filters.incoming)
async def tts_command(client, message):
    if len(message.command) > 1:
        ...
        ask = await message.reply_text("🔊 <b>ᴛᴇxᴛ ᴛᴏ sᴘᴇᴇᴄʜ</b>\n\nPlease send the text you want to convert to audio:", parse_mode=enums.ParseMode.HTML)
        try:
            reply = await client.listen(message.chat.id, filters.text, timeout=30)
            text = reply.text
        except:
            return await ask.edit("⏰ Timed out. Please try again.")
    m = await message.reply_text("🔊 ɢᴇɴᴇʀᴀᴛɪɴɢ ᴀᴜᴅɪᴏ...")
    try:
        tts = gTTS(text=text, lang="en")
        tts.save("tts_output.mp3")
        await message.reply_audio(
            audio="tts_output.mp3",
            caption="🔊 <b>ᴛᴇxᴛ ᴛᴏ sᴘᴇᴇᴄʜ</b> | @Astro_AF_bot",
            parse_mode=enums.ParseMode.HTML
        )
...
@Client.on_message(filters.command("share") & filters.incoming)
async def share_command(client, message):
    if len(message.command) > 1:
        ...
        ask = await message.reply_text("📤 <b>sʜᴀʀᴇ ᴛᴇxᴛ</b>\n\nPlease send the text you want to share:", parse_mode=enums.ParseMode.HTML)
        try:
            reply = await client.listen(message.chat.id, filters.text, timeout=30)
            text = reply.text
        except:
