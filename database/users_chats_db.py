...
# ─── In-memory cache ───────────────────────────────────────────
# Stores {chat_id: (settings_dict, timestamp)}
...
class Database:
    ...
    async def add_clone_bot(self, bot_id, user_id, bot_token):
        settings = {
            'bot_id': bot_id, 'bot_token': bot_token, 'user_id': user_id,
            'url': None, 'api': None, 'tutorial': None, 'update_channel_link': None
        }
    ...
    async def update_settings(self, id, settings):
        ...
        _settings_cache.pop(int(id), None)
        await self.grp.update_one({'id': int(id)}, {'$set': {'settings': settings}})

    async def get_settings(self, id):
        id = int(id)
        # Check cache first
        cached = _settings_cache.get(id)
        if cached:
            settings, ts = cached
        ...
        # Store in cache
        _settings_cache[id] = (settings, time.time())
        return settings
